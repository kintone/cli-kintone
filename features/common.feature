Feature: common test cases
  ...
  Scenario: CliKintoneTest-13 Should return the error message when the app is in a guest space
    Given Load app ID of app "app_in_guest_space" as env var: "APP_ID"
    When I run the command with args "record export --base-url $$TEST_KINTONE_BASE_URL --app $APP_ID --username $$TEST_KINTONE_USERNAME --password $$TEST_KINTONE_PASSWORD"
    Then I should get the exit code is non-zero
