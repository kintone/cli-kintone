...
type SupportedExtension = keyof typeof BASE64_FILE_CONTENTS;
...
export const generateFile = async (
  ...
  if (Object.keys(BASE64_FILE_CONTENTS).indexOf(fileExtension) === -1) {
    throw new Error(`File extension "${fileExtension}" is not supported.`);
  }

  const base64Content =
    BASE64_FILE_CONTENTS[fileExtension as SupportedExtension];
